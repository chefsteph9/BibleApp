...
namespace verseTest
{
    ...
    public async Task<String> apiURI(int book, int chapter, int verseStart, int verseEnd)
    {
        string URI = "http://labs.bible.org/api/?passage=";

        string passage = URI + book.ToString() + "+" + chapter.ToString() + ":" + verseStart.ToString() + "-" + verseEnd.ToString() + "&formatting=plain";
    ...
    public class VerseAddPage : ContentPage
    {
        ...
        public VerseAddPage()
        {
            ...
            {
                passage = apiURI(bibleName, chapter, verseStart, verseEnd);
